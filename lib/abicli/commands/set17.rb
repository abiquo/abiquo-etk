if ARGV[0] == 'set'
  ...
  def set_nfs_repository(val)
    config = IniParser.parse(File.readlines('/opt/abiquo/config/abiquo.properties'))
    config.save('/opt/abiquo/config/abiquo.properties.bak')
    config['server']['abiquo.appliancemanager.repositoryLocation'] = val
    config['server']['abiquo.virtualfactory.xenserver.repositoryLocation'] = val
    config['server']['abiquo.virtualfactory.vmware.repositoryLocation'] = val
    config['server']['abiquo.virtualfactory.hyperv.repositoryLocation'] = val
